...
import { Activity, User, MapPin, LogOut, Shield, ChevronDown, Hexagon } from 'lucide-react';
    ...
    return (
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-700 pb-8">
            ...
            <div className="flex items-center gap-3">
                <button
                    onClick={openDrawer}
                    className="relative p-2.5 rounded-xl bg-slate-800/50 border border-slate-700 hover:border-amber-500/50 hover:bg-amber-500/10 transition-all group"
                    title="消息中心"
                >
                    <Hexagon className="w-5 h-5 text-amber-400 group-hover:text-amber-300 transition-colors" strokeWidth={1.75} />
                    {unreadCount > 0 && (
                        <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-red-500 text-white text-[10px] font-bold flex items-center justify-center border-2 border-slate-900 shadow-lg shadow-red-500/30 animate-pulse">
                            {displayUnread}
                        </span>
                    )}
                </button>
